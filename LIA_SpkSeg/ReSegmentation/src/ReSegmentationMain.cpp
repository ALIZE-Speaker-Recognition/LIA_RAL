...
//  cc.addStringParam("labelSelectedFrames",false,true,"only the frames from segments with this label will be used");
  ...
  cc.addStringParam("loadSegmentationExtension",true,true,"defines the extension to load the files of the label");
...
//  cc.addBooleanParam("aggregateSegment",false,false,"if set to true, aggregate segments after each class");
  cc.addIntegerParam("iterationNb",false,true,"number of iterations for decoding adaptation pass (default=20)");
  ...
  cc.addFloatParam("gammaTransition",true,true,"defines the value gamma for the state transition computation method");
...
//  cc.addStringParam("selectionMethod",false,true,"defines a selection mathod : firstSegFound, Length, limitedLength or Linear (default)");
//  cc.addIntegerParam("selectionLength",true,true,"defines a size of the selected trials");
  cc.addIntegerParam("speakerMinTime",true,true,"defines a minimal length of speaker speech");
//  cc.addIntegerParam("limitSpeaker",false,true,"defines the maximal number of segments by file (default=99999)");
  ...
  cc.addBooleanParam("saveMixture",false,true,"if set to true, save the speaker model (default=false)");
  ...
  try {
      ...
      if (cmdLine.displayHelpRequired()){
        cout << "********************************" << endl;
        cout << "********* ReSegmentation *******" << endl;
        cout << "********************************" << endl;
        cout << endl;
        cout << "This program is used as a second step for segmentation. It refines the segmentation outputs and removes irrelevant speakers." << endl;
        cout << "" << endl;
        cout << cc.getParamList()<<endl;
        return 0;  
      }
      if (cmdLine.displayVersionRequired()){
        cout << "ReSegmentation - Version 2" << endl;
        return 0;  
      } 
      Config tmp;
      ...
      launchReSegmentationProcess(config);
    }
